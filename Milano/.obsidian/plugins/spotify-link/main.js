...
function getTrackMessage(data, artists, template) {
  ...
  return template.replace(/{{ song_name }}|{{song_name}}/g, track.name).replace(
    ...
    `![${track.album.name}](${(_a = track.album.images[0]) == null ? void 0 : _a.url})`
  ).replace(
    /{{ album_cover_medium }}|{{album_cover_medium}}/g,
    `![${track.album.name}](${(_b = track.album.images[1]) == null ? void 0 : _b.url})`
  ).replace(
    /{{ album_cover_small }}|{{album_cover_small}}/g,
    `![${track.album.name}](${(_c = track.album.images[2]) == null ? void 0 : _c.url})`
  ).replace(
    /{{ album_cover_link_large }}|{{album_cover_link_large}}/g,
    `[Cover - ${track.album.name}](${(_d = track.album.images[0]) == null ? void 0 : _d.url})`
  ).replace(
    /{{ album_cover_link_medium }}|{{album_cover_link_medium}}/g,
    `[Cover - ${track.album.name}](${(_e = track.album.images[1]) == null ? void 0 : _e.url})`
  ).replace(
    /{{ album_cover_link_small }}|{{album_cover_link_small}}/g,
    `[Cover - ${track.album.name}](${(_f = track.album.images[2]) == null ? void 0 : _f.url})`
  ).replace(
    ...
    `${(_g = track.album.images[0]) == null ? void 0 : _g.url}`
  ).replace(
    /{{ album_cover_url_medium }}|{{album_cover_url_medium}}/g,
    `${(_h = track.album.images[1]) == null ? void 0 : _h.url}`
  ).replace(
    /{{ album_cover_url_small }}|{{album_cover_url_small}}/g,
    `${(_i = track.album.images[2]) == null ? void 0 : _i.url}`
    ...
    (_j = track.artists[0]) == null ? void 0 : _j.href
  );
}
function getRecentlyPlayedTrackMessage(data, artists, template) {
  var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j;
...
async function processRecentlyPlayedTracks(clientId, clientSecret, data, template = `'{{ song_name }}' by {{ artists }} from {{ album }} released in {{ album_release }} @ {{ played_at }}`) {
  const messages = [];
  if (data && data.items) {
...
async function getRecentlyPlayedTracks(clientId, clientSecret, url = null, recentlyPlayed = null) {
  ...
  try {
    ...
    const response = await (0, import_obsidian.requestUrl)({
      url: url || `${SPOTIFY_API_BASE_ADDRESS}/me/player/recently-played?limit=${LIMIT}&after=${beginningOfDayEpochTime}`,
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`
      }
...
var SpotifyLinkPlugin = class extends import_obsidian4.Plugin {
  ...
  async getName(track) {
    ...
    if ((_a = track == null ? void 0 : track.item) == null ? void 0 : _a.name) {
      name = (_b = track == null ? void 0 : track.item) == null ? void 0 : _b.name;
      if (this.settings.appendArtistNames) {
        const artists = await Promise.all(
          track.item.artists.map(
            (artist) => getArtist(
...
`;
    ...
    const filename = `${(0, import_obsidian4.normalizePath)(
      `/${parent}/${await this.getName(track)}`
    ).replace(/[:|.]/g, "_")}.md`;
